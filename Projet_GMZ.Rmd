        ...
        if(i != 0 | j != 0 ){ #on ne veut pas du pixel 0:0
          if (i >= 0 & j>=0 & i <width & j<height){ #on veut rester dans l'image
            if(isFALSE(tableau[i,j])){  #on veut pas prendre d'autres pixels blancs
              ...
              compte = compte +1
              if(compte  == k){
                result = c() #peut-être rajouter des poids pour les pixels qui sont loins avec distance (genre *(1/distance))
                result$rouge = mean(tab_list_R)  #moyenne rouge des voisins (on remplace mean par sum et on divise par k)
